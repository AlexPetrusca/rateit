...
- The Tourney Create button opens a two-step drawer (Match/Tourney → Live/Historical). A match runs like a 1-net live tournament but is flagged `isMatch`: still rated (Elo counts), excluded from the tournaments list and win-total stats, its live round starts with everyone benched (no auto-pairing — drag onto the net), and the detail screen shows a game-by-game log with per-game Elo change instead of a scoreboard. `isMatch` is a `tourney_tournaments` column separate from `mode`. The older 3-screen DUPR match flow stays wired but dormant (Create is admin-only).
