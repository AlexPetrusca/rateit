...
- The Tourney list's admin **Create** button opens a **two-step drawer**: first **Match · Tourney**, then (swapped in place) **Live · Historical**, then the create screen (`tourney/new`) with `{kind, flow}`. Live is always dated today (no date field); Historical keeps its date picker. A **match** runs exactly like a 1-net live tournament (same create inputs, same round-by-round scoring) but is flagged `isMatch` on the tournament: it's **still rated (Elo counts)** yet excluded from the tournaments list and all tournament/win-total stats, its live round **does not auto-generate pairings** (all active players start benched — the admin drags who's playing onto the one net), and the detail screen shows a **log** (each game's teams, score, and Elo change, newest first) instead of a scoreboard. `isMatch` is a column on `tourney_tournaments`, separate from `mode` (LIVE/HISTORICAL). The older user-facing 3-screen DUPR match flow (`tourney/match`, `POST /api/tourney/matches`) remains wired but has no current entry point (Create is admin-only).
