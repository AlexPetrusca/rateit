...
- The Tourney list's admin **Create** button opens a popup with three choices — **Historical**, **Live**, and **Match**. Historical and Live open the existing create screen (`tourney/new`, no in-screen Live/Historical toggle): Live is always dated today (no date field), Historical keeps its date picker. **Match** opens a three-step flow (`tourney/match`): Who Played (two doubles teams, current user prefilled), the score (Standard/Rally, with game rows that auto-append as you fill them — each game is one rated result), and match details (location, date, optional event). A match is persisted as a hidden `mode=MATCH` tournament so it feeds the shared Elo timeline (interleaved by date then creation order) without appearing in the tournaments list or counting toward tournaments-played/placement stats.
