...
public class TourneyController {
    ...
    public ResponseEntity<List<TourneyMyMatchDto>> getMyMatches(JwtAuthenticationToken token) {
        return ResponseEntity.ok(tourneyService.getMyMatches(token.getToken().getSubject()));
    }
    ...
    @GetMapping("/live/me")
    public ResponseEntity<TourneyLiveBannerDto> getMyLiveTournament(JwtAuthenticationToken token) {
        TourneyLiveBannerDto live = tourneyService.getMyLiveTournament(token.getToken().getSubject());
